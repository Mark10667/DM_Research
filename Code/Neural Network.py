...
CONFIG = [
    dict(game="CartPole-v0",
         n_feature=4, n_action=2, continuous_a=[False], ep_max_step=700, eval_threshold=600),
...
def get_reward(shapes, params, env, ep_max_step, continuous_a, seed_and_id=None, control_var = None):
    # perturb parameters using seed
    if seed_and_id is not None and control_var is not None:
        if control_var == 'antithetic':
            seed, k_id = seed_and_id
            np.random.seed(seed)
            params += sign(k_id) * SIGMA * np.random.randn(params.size)
        elif control_var == 'vanilla':
            seed, k_id = seed_and_id
            np.random.seed(seed)
...
def train(net_shapes, net_params, optimizer, utility, pool,control_var=None,method=None):
    if control_var == 'antithetic':
        ...
        jobs = [pool.apply_async(get_reward, (net_shapes, net_params, env, CONFIG['ep_max_step'], CONFIG['continuous_a'],
                                              [noise_seed[k_id], k_id],control_var,)) for k_id in range(N_KID * 2)]
        ...
        if method =='SGD':
            ...
            v = optimizer.get_gradients(cumulative_update / (2 * N_KID * SIGMA))
        elif method == "lasso":
            lasso_method = linear_model.Lasso(0.1)
            lasso_method.fit(params,rewards)
            v=lasso_method.coef_
        elif method == "ridge":
            ridge_cv = RidgeCV(cv=5)
            lrr = ridge_cv.fit(params, rewards)
            v = lrr.coef_
        elif method == "lp-decoding":
            lr = LinearRegression()
            lr.fit(params, rewards)
            v = lr.coef_
        else:
            print("method not available.")
            return
        return net_params + v, rewards

    elif control_var == 'fd-style':
        # pass seed instead whole noise matrix to parallel will save your time
        noise_seed = np.random.randint(0, 2 ** 32 - 1, size=N_KID, dtype=np.uint32).repeat(2)  # mirrored sampling

        # distribute training in parallel
        jobs = [pool.apply_async(get_reward, (net_shapes, net_params, env, CONFIG['ep_max_step'], CONFIG['continuous_a'],
                                              [noise_seed[k_id], k_id],control_var,)) for k_id in range(2 * N_KID)]
        out = np.array([j.get() for j in jobs])
        rewards = out[:,1]
        params = []
        for i in out[:,0]:
        ...
        if method == 'SGD':
            for ui, k_id in enumerate(kids_rank):
                np.random.seed(noise_seed[k_id])  # reconstruct noise using seed
                cumulative_update += sign(k_id) *utility[ui] * np.random.randn(net_params.size)
            v = optimizer.get_gradients(cumulative_update / (N_KID * SIGMA))
        elif method == "lasso":
            lasso_method = linear_model.Lasso(0.1)
            lasso_method.fit(params,rewards)
            v=lasso_method.coef_
        ...
        # distribute training in parallel
        jobs = [pool.apply_async(get_reward, (net_shapes, net_params, env, CONFIG['ep_max_step'], CONFIG['continuous_a'],
                                              [noise_seed[k_id], k_id],control_var,)) for k_id in range(N_KID)]
        out = np.array([j.get() for j in jobs])
        rewards = out[:,1]
        params = []
        for i in out[:,0]:
        ...
        if method == 'SGD':
            for ui, k_id in enumerate(kids_rank):
                np.random.seed(noise_seed[k_id])  # reconstruct noise using seed
                cumulative_update += utility[ui] * np.random.randn(net_params.size)
            v = optimizer.get_gradients(cumulative_update / (N_KID * SIGMA))
        elif method == "lasso":
            lasso_method = linear_model.Lasso(0.5)
            lasso_method.fit(params,rewards)
            v=lasso_method.coef_*0.2
        elif method == "ridge":
            ridge_cv = RidgeCV(cv=5)
            lrr = ridge_cv.fit(params, rewards)
            v = lrr.coef_
...
if __name__ == "__main__":
    ...
    control_var = ['antithetic']
    regression_method = ['SGD']
    ...
    # plot
    for i in outcome:
        plt.plot(np.arange(0, len(outcome[i])), outcome[i],label = i)
    plt.legend()
    plt.show()
